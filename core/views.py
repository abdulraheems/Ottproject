...
def download_video(request, video_id):
    ...
    if video in user.downloaded_videos.all():
        user.downloaded_videos.remove(video)
    else:
        user.downloaded_videos.add(video)
    return HttpResponseRedirect(request.META.get("HTTP_REFERER"))
...
def downloadedVideos(request):
    try:
        video = request.user.profile.downloaded_videos.all()
    except:
        video = None
    context = {
        "video":video
    }
    return render(request, "downloaded-video.html", context)
